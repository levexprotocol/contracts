...
contract QueryHelper {
    ...
    function getTraderPositons(ILevex levex, uint16 marketId, address[] calldata traders, bool[] calldata longTokens, bytes calldata dexData) external view returns (PositionVars[] memory results){
        results = new PositionVars[](traders.length);
        ILevex.MarketVar memory market = levex.markets(marketId);
        for (uint i = 0; i < traders.length; i++) {
            PositionVars memory item;
            Types.Trade memory trade = levex.activeTrades(traders[i], marketId, longTokens[i]);
            if (trade.held == 0) {
                results[i] = item;
                continue;
            }
            ...
            (item.marginRatio,,,item.marginLimit) = levex.marginRatio(traders[i], marketId, longTokens[i], dexData);
    ...
    //offchain call
    function getTraderLiqs(Ilevex levex, uint16 marketId, address[] calldata traders, bool[] calldata longTokens, bytes calldata dexData) external returns (LiqVars[] memory results){
        results = new LiqVars[](traders.length);
        LiqReqVars memory reqVar;
        reqVar.levex = levex;
        reqVar.marketId = marketId;
        reqVar.dexData = dexData;
        Ilevex.MarketVar memory market = reqVar.levex.markets(reqVar.marketId);
        Ilevex.AddressConfig memory adrConf = reqVar.levex.addressConfig();
        Ilevex.CalculateConfig memory calConf = reqVar.levex.calculateConfig();
        ...
        levex.updatePrice(marketId, dexData);
        ...
        for (uint i = 0; i < traders.length; i++) {
            ...
            LiqVars memory item;
            Types.Trade memory trade = reqVar.levex.activeTrades(reqVar.owner, reqVar.marketId, reqVar.longToken);
            if (trade.held == 0) {
            ...
            (item.currentMarginRatio, item.cAvgMarginRatio, item.hAvgMarginRatio, item.marginLimit) = reqVar.levex.marginRatio(reqVar.owner, reqVar.marketId, reqVar.longToken, reqVar.dexData);
    ...
    // offchain call
    function calPriceCAvgPriceHAvgPrice(Ilevex levex, uint16 marketId, address desToken, address quoteToken, uint32 secondsAgo, bytes memory dexData) external
    returns (uint price, uint cAvgPrice, uint256 hAvgPrice, uint8 decimals, uint256 timestamp){
        Ilevex.AddressConfig memory adrConf = levex.addressConfig();
    ...
    function getLiqCallData(Ilevex levex, IV3Quoter v3Quoter, uint16 marketId, uint16 slippage, address trader, bool longToken, bytes memory dexData)
    external returns (uint minBuy, uint maxSell)
    {
        Ilevex.MarketVar memory market = levex.markets(marketId);
        Types.Trade memory trade = levex.activeTrades(trader, marketId, longToken);
        LiqCallVars memory callVars;
        // held -> amount
        callVars.totalHeld = levex.totalHelds(longToken ? market.token1 : market.token0);
        callVars.balance = IERC20(longToken ? market.token1 : market.token0).balanceOf(address(levex));
        ...
        Ilevex.AddressConfig memory adrConf = levex.addressConfig();
        Ilevex.CalculateConfig memory calConf = levex.calculateConfig();
        ...
        callVars.txTax = levex.taxes(marketId, callVars.buyToken, 0);
        callVars.buyTax = levex.taxes(marketId, callVars.buyToken, 2);
        callVars.sellTax = levex.taxes(marketId, callVars.sellToken, 1);
    ...
    function getPoolDetails(Ilevex levex, uint16[] calldata marketIds, LPoolInterface[] calldata pools) external view returns (PoolVars[] memory results){
        results = new PoolVars[](pools.length);
        for (uint i = 0; i < pools.length; i++) {
            LPoolInterface pool = pools[i];
            Ilevex.MarketVar memory market = levex.markets(marketIds[i]);
    ...
    function getlevexBalances(Ilevex levex, address[] calldata tokens) external view returns (Balance[] memory results){
        results = new Balance[](tokens.length);
        for (uint i = 0; i < tokens.length; i++) {
            uint balance = IERC20(tokens[i]).balanceOf(address(levex));
            uint totalHelds = levex.totalHelds(tokens[i]);
