...
import "./Types.sol";
import "./LevexV1Lib.sol";

/// @title Levexerage margin trade logic
/// @author Levexerage
...
contract LevexV1 is DelegateInterface, Adminable, ReentrancyGuard, LevexInterface, LevexStorage {
    ...
    /// @param _xlvx Address of XlvxDelegator.
    ...
    function initialize(
        ...
        addressConfig.xlvx = _xlvx;
        for (uint i = 0; i < _supportDexs.length; i++) {
            supportDexs[_supportDexs[i]] = true;
        }
        LevexV1Lib.setCalculateConfig(22, 33, 2500, 5, 25, 25, 5000e18, 500, 5, 60, calculateConfig);
    }
    ...
    function addMarket(
        ...
        LevexV1Lib.addMarket(pool0, pool1, marginLimit, dexData, marketId, markets, calculateConfig, addressConfig, supportDexs, taxes);
        numPairs ++;
        return marketId;
    }
    ...
    function _marginTradeFor(address trader, uint16 marketId, bool longToken, bool depositToken, uint deposit, uint borrow, uint minBuyAmount, bytes memory dexData) internal returns (uint256 newHeld){
        ...
        Types.MarketVars memory vars = LevexV1Lib.toMarketVar(longToken, true, markets[marketId]);
        bytes memory calPriceDexData = LevexV1Lib.getCalPriceDexData(dexData, vars.dexs[0]);
        {
            Types.Trade storage t = activeTrades[trader][marketId][longToken];
            LevexV1Lib.verifyTrade(vars, longToken, depositToken, deposit, borrow, dexData, addressConfig, t, msg.sender == opLimitOrder ? false : true);
        ...
        if (borrow > 0) {
            {
                uint balance = LevexV1Lib.balanceOf(vars.sellToken);
                vars.sellPool.borrowBehalf(trader, borrow);
                borrowed = LevexV1Lib.balanceOf(vars.sellToken).sub(balance);
            }
        ...
        tv.newHeld = LevexV1Lib.amountToShare(tv.newHeld, tv.totalHeld, vars.reserveBuyToken);
        ...
        require(isPositionHealthy(
                trader,
                true,
                LevexV1Lib.shareToAmount(trade.held, totalHelds[address(vars.buyToken)], LevexV1Lib.balanceOf(vars.buyToken)),
                vars,
                calPriceDexData
            ), "PNH");
    ...
    function _closeTradeFor(address trader, uint16 marketId, bool longToken, uint closeHeld, uint minOrMaxAmount, bytes memory dexData) internal returns (uint256){
        Types.Trade storage trade = activeTrades[trader][marketId][longToken];
        Types.MarketVars memory marketVars = LevexV1Lib.toMarketVar(longToken, false, markets[marketId]);
        bool depositToken = trade.depositToken;

        //verify
        require(closeHeld <= trade.held, "CBH");
        require(trade.held != 0 && trade.lastBlockNum != block.number && LevexV1Lib.isInSupportDex(marketVars.dexs, dexData.toDexDetail()), "HI0");
        ...
        uint closeAmount = LevexV1Lib.shareToAmount(closeHeld, totalHelds[address(marketVars.sellToken)], marketVars.reserveSellToken);
        ...
        closeTradeVars.borrowed = LevexV1Lib.borrowCurrent(marketVars.buyPool, trader);
        ...
        if (depositToken != longToken) {
            ...
            LevexV1Lib.repay(marketVars.buyPool, trader, closeTradeVars.repayAmount);
            ...
            uint balance = LevexV1Lib.balanceOf(marketVars.buyToken);
            ...
            closeTradeVars.sellAmount = flashBuy(marketId,
                ...
                dexData,
                LevexV1Lib.toBytes(marketVars.dexs[0]));
            closeTradeVars.receiveAmount = LevexV1Lib.balanceOf(marketVars.buyToken).sub(balance);
            require(closeTradeVars.receiveAmount >= closeTradeVars.repayAmount, "ISR");

            LevexV1Lib.repay(marketVars.buyPool, trader, closeTradeVars.repayAmount);
            closeTradeVars.depositReturn = closeTradeVars.closeAmountAfterFees.sub(closeTradeVars.sellAmount);
            require(LevexV1Lib.balanceOf(marketVars.sellToken) >= closeTradeVars.depositReturn, "ISB");
            doTransferOut(trader, marketVars.sellToken, closeTradeVars.depositReturn);
        }

        uint repayed = closeTradeVars.borrowed.sub(LevexV1Lib.borrowCurrent(marketVars.buyPool, trader));
        ...
        if (dexData.isUniV2Class()) {
            updatePrice(address(marketVars.buyToken), address(marketVars.sellToken), LevexV1Lib.getCalPriceDexData(dexData, marketVars.dexs[0]));
        }
    ...
    function payoffTrade(uint16 marketId, bool longToken) external payable override nonReentrant {
        ...
        uint closeAmount = LevexV1Lib.shareToAmount(heldAmount, totalHelds[address(marketVars.sellToken)], marketVars.reserveSellToken);
        uint borrowed = LevexV1Lib.borrowCurrent(marketVars.buyPool, msg.sender);

        //first transfer token to Levexe, then repay to pool, two transactions with two tax deductions
        ...
        LevexV1Lib.repay(marketVars.buyPool, msg.sender, transferAmount);
    ...
    function liquidate(address owner, uint16 marketId, bool longToken, uint minBuy, uint maxSell, bytes memory dexData) external override nonReentrant onlySupportDexExclude1inch(dexData) {
        ...
        require(trade.held != 0 && trade.lastBlockNum != block.number && LevexV1Lib.isInSupportDex(marketVars.dexs, dexData.toDexDetail()), "HI0");
        uint closeAmount = LevexV1Lib.shareToAmount(trade.held, totalHelds[address(marketVars.sellToken)], marketVars.reserveSellToken);
        ...
        liquidateVars.borrowed = LevexV1Lib.borrowCurrent(marketVars.buyPool, owner);
        ...
        if (buySuccess) {
            ...
            liquidateVars.receiveAmount = LevexV1Lib.balanceOf(marketVars.buyToken).sub(marketVars.reserveBuyToken);
            LevexV1Lib.repay(marketVars.buyPool, owner, liquidateVars.borrowed);
            ...
            if (liquidateVars.receiveAmount >= liquidateVars.borrowed) {
                ...
                marketVars.buyPool.repayBorrowEndByLevex(owner, liquidateVars.finalRepayAmount);
            }
        }
    ...
    function marginRatio(address owner, uint16 marketId, bool longToken, bytes memory dexData) external override onlySupportDex(dexData) view returns (uint current, uint cAvg, uint hAvg, uint32 limit) {
        (current, cAvg, hAvg, limit) = LevexV1Lib.marginRatio(marketId, owner, longToken, dexData);
    }

    /// @notice Update price on Dex.
    /// @param dexData Index and fee rate for the trading Dex.
    function updatePrice(uint16 marketId, bytes memory dexData) external override {
        LevexV1Lib.updatePrice(markets[marketId], dexData);
    }
    ...
    function setCalculateConfig(uint16 defaultFeesRate,
        ...
        LevexV1Lib.setCalculateConfig(defaultFeesRate, insuranceRatio, defaultMarginLimit, priceDiffientRatio, updatePriceDiscount,
    ...
    function setMarketConfig(uint16 marketId, uint16 feesRate, uint16 marginLimit, uint16 priceDiffientRatio, uint32[] memory dexs) external override onlyAdmin() {
        LevexV1Lib.setMarketConfig(feesRate, marginLimit, priceDiffientRatio, dexs, markets[marketId]);
    ...
    function moveInsurance(uint16 marketId, uint8 poolIndex, address to, uint amount) external override nonReentrant() onlyAdmin() {
        ...
        LevexV1Lib.moveInsurance(market, poolIndex, to, amount, totalHelds);
    }
    ...
    function reduceInsurance(uint totalRepayment, uint remaining, uint16 marketId, bool longToken, address token, uint reserve) internal returns (uint maxCanRepayAmount) {
        Types.Market storage market = markets[marketId];
        return LevexV1Lib.reduceInsurance(totalRepayment, remaining, longToken, token, reserve, market, totalHelds);
    }
    ...
    function feesAndInsurance(address trader, uint tradeSize, address token, uint16 marketId, uint reserve, uint totalHeld) internal returns (uint) {
        Types.Market storage market = markets[marketId];
        return LevexV1Lib.feeAndInsurance(trader, tradeSize, token, addressConfig.xlvx, totalHeld, reserve, market, totalHelds, calculateConfig);
    }
    ...
    function flashSell(address buyToken, address sellToken, uint sellAmount, uint minBuyAmount, bytes memory data) internal returns (uint){
        return LevexV1Lib.flashSell(buyToken, sellToken, sellAmount, minBuyAmount, data, addressConfig.dexAggregator, router1inch);
    }
    ...
    function flashBuy(uint16 marketId, address buyToken, address sellToken, uint buyAmount, uint maxSellAmount, uint closeAmount, bytes memory data, bytes memory marketDefaultDex) internal returns (uint){
        ...
        return LevexV1Lib.flashBuy(buyTax, sellTax, router1inch, addressConfig.dexAggregator, buyToken, sellToken, buyAmount, maxSellAmount, closeAmount, data, marketDefaultDex);
    }
    ...
    function isPositionHealthy(
        ...
        return LevexV1Lib.isPositionHealthy(owner, isOpen, amount, vars, dexData);
    }
