...
/// @dev DexDataFormat addPair = byte(dexID) + bytes3(feeRate) + bytes(arrayLength) + byte3[arrayLength](trasferFeeRate Lpool <-> Levex) 
/// + byte3[arrayLength](transferFeeRate Levex -> Dex) + byte3[arrayLength](Dex -> transferFeeRate Levex)
