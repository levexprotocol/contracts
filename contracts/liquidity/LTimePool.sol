...
contract LTimePool is DelegateInterface, Adminable, LPoolInterface, Exponential, ReentrancyGuard {
    ...
    function initialize(
        ...
        require(accrualBlockTimestamp == 0 && borrowIndex == 0, "inited once");
        ...
        // Initialize block timestamp and borrow index (block timestamp mocks depend on controller being set)
    ...
    function repayBorrowEndByLevex(address borrower, uint repayAmount) external override nonReentrant {
        accrueInterest();
    ...
    function calCurrentBorrowIndex() internal view returns (MathError, uint) {
        /* Remember the initial timestamp number */
        uint currentBlockTimestamp = getBlockTimestamp();
        uint accrualBlockTimestampPrior = accrualBlockTimestamp;
        uint borrowIndexNew;
        /* Short-circuit accumulating 0 interest */
    ...
    function mintFresh(address minter, uint mintAmount, bool isDelegete) internal sameTimestamp returns (uint) {
    ...
    function redeemFresh(address payable redeemer, uint redeemTokensIn, uint redeemAmountIn) internal sameTimestamp {
    ...
    function borrowFresh(address payable borrower, address payable payee, uint borrowAmount) internal sameTimestamp {
    ...
    function repayBorrowFresh(address payer, address borrower, uint repayAmount, bool isEnd) internal sameTimestamp returns (uint) {
    ...
    modifier sameTimestamp() {
        require(accrualBlockTimestamp == getBlockTimestamp(), 'not same timestamp');
        _;
    }
}
