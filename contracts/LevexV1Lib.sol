...
library LevexV1Lib {
    ...
    function addMarket(
        ...
        LevexStorage.CalculateConfig storage config,
        LevexStorage.AddressConfig storage addressConfig,
    ...
    function marginRatio(
        ...
        (market.pool0, market.pool1, market.token0, market.token1, market.marginLimit,,,,,) = (LevexStorage(address(this))).markets(marketId);
        tokenToLong = longToken ? market.token1 : market.token0;
        limit = market.marginLimit;
        (,uint amount,,) = LevexStorage(address(this)).activeTrades(owner, marketId, longToken);
        amount = shareToAmount(
            amount,
            LevexStorage(address(this)).totalHelds(tokenToLong),
    ...
    function isPositionHealthy(
        ...
        (,,,,,,,,uint8 maxLiquidationPriceDiffientRatio,) = LevexStorage(address(this)).calculateConfig();
        if (isOpen) {
    ...
    function feeAndInsurance(
        ...
        LevexStorage.CalculateConfig storage calculateConfig
        ...
        IERC20(token).safeTransfer(lvx, newFees.sub(newInsurance));

        newInsurance = levexV1Lib.amountToShare(newInsurance, totalHeld, reserve);
    ...
    function verifyTrade(Types.MarketVars memory vars, bool longToken, bool depositToken, uint deposit, uint borrow,
        bytes memory dexData, levexStorage.AddressConfig storage addressConfig, Types.Trade storage trade, bool convertWeth) external view {
    ...
    function marginRatioPrivate(
        ...
        (DexAggregatorInterface dexAggregator,,,) = levexStorage(address(this)).addressConfig();
        (,,,,,,,,,uint16 twapDuration) = levexStorage(address(this)).calculateConfig();
