...
const {toWei, toETH, assertPrint, approxAssertPrint, approxPrecisionAssertPrint, assertThrows, createUniswapV2Factory, createEthDexAgg, createXOLE} = require("./utils/LevexUtil");
...
contract("LPoolDelegator", async accounts => {
    ...
    it("badDebtsAmount test with one seconds to produce a block", async () => {
        ...
        await controller.setLevex(accounts[1]);
        //Borrow money 5000
        ...
        let tx = await erc20Pool.repayBorrowEndByLevex(accounts[2], 1000 * 1e10, {from: accounts[1]});
    ...
    it("repayBorrowEndByLevex test", async () => {
        let controller = await utils.createController(accounts[0]);
        let createPoolResult = await utils.createTimePool(accounts[0], controller, admin);
        let erc20Pool = createPoolResult.pool;
        let testToken = createPoolResult.token;
    ...
    it("reverses test ", async () => {
        ...
        if(totalBorrows.toString()=='0') {
            ...
            //reduce reserves
            await erc20Pool.reduceReserves(accounts[1], reserves.toString());
            let reservesAfterReduce = await erc20Pool.totalReserves();
            assert.equal(reservesAfterReduce.toString(), '0');

            assert.equal(toETH(await erc20Pool.supplyRatePerBlock()), 0);
            assert.equal((await erc20Pool.exchangeRateStored()).toString(), 1e18);
            assert.equal((await erc20Pool.getCash()).toString(), '0');

            // add reserves
            await erc20Pool.addReserves('100000000000000000');
            approxPrecisionAssertPrint("totalReserves", '100000000000000000', (await erc20Pool.totalReserves()).toString(), 8);
            approxPrecisionAssertPrint("cash", '100000000000000000', (await erc20Pool.getCash()).toString(), 8);
        }
    })
