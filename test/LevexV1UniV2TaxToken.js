...
contract("Levex UniV2", async accounts => {
    // components
    let Levex;
    let LVX;
    let treasury;
    ...
    it("should long token0 with deposit token0 and closed properly", async() =>{
        ...
        m.log("-- margin trade...")
        await Levex.marginTrade(pairId, false, false, deposit, borrow, 0, Uni2DexData, {from: trader});
        let PriceData01 = await dexAgg.getPriceCAvgPriceHAvgPrice(weth.address, token0.address, 60, Uni2DexData);
        let marginRatio = await Levex.marginRatio(trader, pairId, 0, Uni2DexData);
        let trade = await Levex.activeTrades(trader, pairId, 0);
        let balance = await token0.balanceOf(Levex.address);
        ...
        await Levex.marginTrade(pairId, false, false, deposit, 0, 0, Uni2DexData, {from: trader});
        let PriceData02 = await dexAgg.getPriceCAvgPriceHAvgPrice(weth.address, token0.address, 60, Uni2DexData);
        marginRatio = await Levex.marginRatio(trader, pairId, 0, Uni2DexData);
        trade = await Levex.activeTrades(trader, pairId, 0);
        balance = await token0.balanceOf(Levex.address);
        ...
        m.log("-- update reward...");
        await token0.transfer(Levex.address, toBN(5e17));
        let tradeAfterUpdateReward = await Levex.activeTrades(trader, pairId, 0);
        balance = await token0.balanceOf(Levex.address);
        ...
        await Levex.closeTrade(pairId, false, toBN(Math.floor(trade.held / 2)), utils.maxUint(), Uni2DexData, {from: trader});
        let PriceData03 = await dexAgg.getPriceCAvgPriceHAvgPrice(weth.address, token0.address, 60, Uni2DexData);
        let tradeAfterClose = await Levex.activeTrades(trader, pairId, 0);
        balance = await token0.balanceOf(Levex.address);
        ...
        await Levex.closeTrade(pairId, false, tradeAfterClose.held, utils.maxUint(), Uni2DexData, {from: trader});
        let PriceData04 = await dexAgg.getPriceCAvgPriceHAvgPrice(weth.address, token0.address, 60, Uni2DexData);
        let tradeAfterCloseAlls = await Levex.activeTrades(trader, pairId, 0);
        balance = await token0.balanceOf(Levex.address);
    ...
    it("should long token0 with deposit token0 and liquidate properly", async() =>{
        ...
        let liquidationTx = await Levex.liquidate(trader, pairId, 0, 0, utils.maxUint(), Uni2DexData, {from: liquidator2});
        marginRatio = await Levex.marginRatio(trader, pairId, 0, Uni2DexData);
        let tradeAfterLiquidate = await Levex.activeTrades(trader, pairId, 0);
        balance = await token0.balanceOf(Levex.address);
        let market = await Levex.markets(pairId);
    ...
    it("should long token0 with deposit eth and closed properly", async() =>{
        ...
        m.log("-- margin trade...")
        await Levex.marginTrade(pairId, false, true, 0, borrow, 0, Uni2DexData, {from: trader, value: deposit});
        let PriceData01 = await dexAgg.getPriceCAvgPriceHAvgPrice(weth.address, token0.address, 60, Uni2DexData);
        let marginRatio = await Levex.marginRatio(trader, pairId, 0, Uni2DexData);
        let trade = await Levex.activeTrades(trader, pairId, 0);
        let balance = await token0.balanceOf(Levex.address);
        ...
        m.log("-- margin trade again...")
        await Levex.marginTrade(pairId, false, true, 0, 0, 0, Uni2DexData, {from: trader, value: deposit});
        let PriceData02 = await dexAgg.getPriceCAvgPriceHAvgPrice(weth.address, token0.address, 60, Uni2DexData);
        marginRatio = await Levex.marginRatio(trader, pairId, 0, Uni2DexData);
        trade = await Levex.activeTrades(trader, pairId, 0);
        balance = await token0.balanceOf(Levex.address);
        ...
        m.log("-- close trade partial...");
        await Levex.closeTrade(pairId, false, toBN(Math.floor(trade.held / 2)), 0, Uni2DexData, {from: trader});
        let PriceData03 = await dexAgg.getPriceCAvgPriceHAvgPrice(weth.address, token0.address, 60, Uni2DexData);
        marginRatio = await Levex.marginRatio(trader, 0, 0, Uni2DexData);
        let tradeAfterClose = await Levex.activeTrades(trader, pairId, 0);
        balance = await token0.balanceOf(Levex.address);
        m.log("PriceData03: \t", JSON.stringify(PriceData03));
        m.log("tradeAfterClose.held:", tradeAfterClose.held);
        ...
        assert.equal(tradeAfterClose.held.toString(), toBN(Math.ceil(trade.held / 2)).toString());
         ...
         m.log("-- close trade all...");
        await Levex.closeTrade(pairId, false, tradeAfterClose.held, 0, Uni2DexData, {from: trader});
        let PriceData04 = await dexAgg.getPriceCAvgPriceHAvgPrice(weth.address, token0.address, 60, Uni2DexData);
        let tradeAfterCloseAlls = await Levex.activeTrades(trader, pairId, 0);
        balance = await token0.balanceOf(Levex.address);
        m.log("PriceData04: \t", JSON.stringify(PriceData04));
    ...
    it("should short token0 with deposit token0 and closed properly", async() =>{
        ...
        let PriceData01 = await dexAgg.getPriceCAvgPriceHAvgPrice(weth.address, token0.address, 60, Uni2DexData);
        let marginRatio = await Levex.marginRatio(trader, pairId, 1, Uni2DexData);
        let trade = await Levex.activeTrades(trader, pairId, 1);
        let balance = await weth.balanceOf(Levex.address);
        m.log("PriceData01: \t", JSON.stringify(PriceData01));
        m.log("Margin Ratio current:", marginRatio.current / 100, "%");
        m.log("Margin Ratio cAvg:", marginRatio.cAvg / 100, "%");
        m.log("Margin Ratio hAvg:", marginRatio.hAvg / 100, "%");
        ...
        m.log("-- margin trade again...")
        await Levex.marginTrade(pairId, true, false, deposit, 0, 0, Uni2DexData, {from: trader});
        let PriceData02 = await dexAgg.getPriceCAvgPriceHAvgPrice(weth.address, token0.address, 60, Uni2DexData);
        marginRatio = await Levex.marginRatio(trader, pairId, 1, Uni2DexData);
        trade = await Levex.activeTrades(trader, pairId, 1);
        balance = await weth.balanceOf(Levex.address);
        m.log("PriceData02: \t", JSON.stringify(PriceData02));
        m.log("Margin Ratio current:", marginRatio.current / 100, "%");
        m.log("Margin Ratio cAvg:", marginRatio.cAvg / 100, "%");
        m.log("Margin Ratio hAvg:", marginRatio.hAvg / 100, "%");
        ...
        m.log("-- close trade partial...");
        await Levex.closeTrade(pairId, true, toBN(Math.floor(trade.held / 2)), 0, Uni2DexData, {from: trader});
        let PriceData03 = await dexAgg.getPriceCAvgPriceHAvgPrice(weth.address, token0.address, 60, Uni2DexData);
        let tradeAfterClose = await Levex.activeTrades(trader, pairId, 1);
        balance = await weth.balanceOf(Levex.address);
        m.log("PriceData03: \t", JSON.stringify(PriceData03));
        m.log("tradeAfterClose.held:", tradeAfterClose.held);
        m.log("balance:", balance);
        ...
        m.log("-- close trade all...");
        await Levex.closeTrade(pairId, true, tradeAfterClose.held, 0, Uni2DexData, {from: trader});
        let PriceData04 = await dexAgg.getPriceCAvgPriceHAvgPrice(weth.address, token0.address, 60, Uni2DexData);
        let tradeAfterCloseAlls = await Levex.activeTrades(trader, pairId, 1);
        balance = await weth.balanceOf(Levex.address);
        m.log("PriceData04: \t", JSON.stringify(PriceData04));
        m.log("tradeAfterCloseAlls.held:", tradeAfterCloseAlls.held);
        m.log("balance:", balance);
    ...
    it("should short token0 with deposit token0 and liquidate properly", async() =>{
        ...
        m.log("-- liquidate with deposit return")
        let liquidationTx = await Levex.liquidate(trader, pairId, true, 0, utils.maxUint(), Uni2DexData, {from: liquidator2});
        marginRatio = await Levex.marginRatio(trader, pairId, 0, Uni2DexData);
        let tradeAfterLiquidate = await Levex.activeTrades(trader, pairId, 1);
        balance = await token0.balanceOf(Levex.address);
        let market = await Levex.markets(pairId);
        m.log("deposit return:", liquidationTx.logs[0].args.depositReturn);
        m.log("tradeAfterLiquidate.held:", tradeAfterLiquidate.held);
    ...
    it("should short token0 with deposit eth and closed properly", async() =>{
        ...
        m.log("-- margin trade...")
        await Levex.marginTrade(pairId, true, true, 0, borrow, 0, Uni2DexData, {from: trader, value: deposit});
        let PriceData01 = await dexAgg.getPriceCAvgPriceHAvgPrice(weth.address, token0.address, 60, Uni2DexData);
        let marginRatio = await Levex.marginRatio(trader, pairId, 1, Uni2DexData);
        let trade = await Levex.activeTrades(trader, pairId, 1);
        let balance = await weth.balanceOf(Levex.address);
        ...
        m.log("-- margin trade again...")
        await Levex.marginTrade(pairId, true, true, 0, 0, 0, Uni2DexData, {from: trader, value: deposit});
        let PriceData02 = await dexAgg.getPriceCAvgPriceHAvgPrice(weth.address, token0.address, 60, Uni2DexData);
        marginRatio = await Levex.marginRatio(trader, pairId, 1, Uni2DexData);
        trade = await Levex.activeTrades(trader, pairId, 1);
        balance = await weth.balanceOf(Levex.address);
        ...
        m.log("-- close trade partial...");
        await Levex.closeTrade(pairId, true, toBN(Math.floor(trade.held / 2)), utils.maxUint(), Uni2DexData, {from: trader});
        let PriceData03 = await dexAgg.getPriceCAvgPriceHAvgPrice(weth.address, token0.address, 60, Uni2DexData);
        let tradeAfterClose = await Levex.activeTrades(trader, pairId, 1);
        balance = await token0.balanceOf(Levex.address);
        m.log("PriceData03: \t", JSON.stringify(PriceData03));
        ...
        m.log("-- close trade all...");
        await Levex.closeTrade(pairId, true, tradeAfterClose.held, utils.maxUint(), Uni2DexData, {from: trader});
        let PriceData04 = await dexAgg.getPriceCAvgPriceHAvgPrice(weth.address, token0.address, 60, Uni2DexData);
        let tradeAfterCloseAlls = await Levex.activeTrades(trader, pairId, 1);
        balance = await token0.balanceOf(Levex.address);
        m.log("PriceData04: \t", JSON.stringify(PriceData04));
    ...
    it("should long token0 with deposit token0 and closed properly with sell tax only", async() =>{
        ...
        await Levex.setTaxRate(pairId, token0.address, 0, 0);
        await Levex.setTaxRate(pairId, token0.address, 2, 0);
    ...
    it("should long token0 with deposit token0 and closed properly with buy tax only", async() =>{
        ...
        await token0.excludeFromFee(trader);
        await Levex.setTaxRate(pairId, token0.address, 0, 0);
        await Levex.setTaxRate(pairId, token0.address, 1, 0);
    ...
    it("should short token0 with deposit token0 and closed properly tax increase after deposit", async() =>{
        ...
        m.log("-- close trade partial...");
        await assertThrows(Levex.closeTrade(pairId, true, toBN(Math.floor(trade.held / 2)), 0, Uni2DexData, {from: trader}));
        await Levex.setTaxRate(pairId, token0.address, 0, 100000);
        await Levex.setTaxRate(pairId, token0.address, 1, 100000);
        await Levex.setTaxRate(pairId, token0.address, 2, 100000);
        await Levex.closeTrade(pairId, true, toBN(Math.floor(trade.held / 2)), 0, Uni2DexData, {from: trader});
        let PriceData03 = await dexAgg.getPriceCAvgPriceHAvgPrice(weth.address, token0.address, 60, Uni2DexData);
        let tradeAfterClose = await Levex.activeTrades(trader, pairId, 1);
        balance = await weth.balanceOf(Levex.address);
    ...
    it("should short token0 with deposit eth and closed properly tax increase after deposit", async() =>{
        ...
        m.log("-- close trade partial...");
        await assertThrows(Levex.closeTrade(pairId, true, toBN(Math.floor(trade.held / 2)), utils.maxUint(), Uni2DexData, {from: trader}));
        await Levex.setTaxRate(pairId, token0.address, 0, 100000);
        await Levex.setTaxRate(pairId, token0.address, 1, 100000);
        await Levex.setTaxRate(pairId, token0.address, 2, 100000);
        await Levex.closeTrade(pairId, true, toBN(Math.floor(trade.held / 2)), utils.maxUint(), Uni2DexData, {from: trader});
