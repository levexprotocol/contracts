...
contract("Levex UniV2", async accounts => {
    ...
    it("Deposit Eth，return eth ", async () => {
        ...
        let updatePrice_tx = await Levex.updatePrice(pairId, Uni2DexData);
        ...
        await Levex.marginTrade(pairId, false, false, 0, borrow, 0, Uni2DexData, {from: trader, value: deposit});
        ...
        await Levex.closeTrade(pairId, false, tradeBefore.held, utils.maxUint(), Uni2DexData, {from: trader});
    ...
    it("LONG Token0, Not Init Price ,Not Succeed ", async () => {
        ...
        m.log("toBorrow from Pool 1: \t", borrow);
        await assertThrows(Levex.marginTrade(0, false, true, deposit, borrow, 0, Uni2DexData, {from: trader}), 'PNH');
    })
    it("LONG Token0, Not Init Price ,60s later Succeed ", async () => {
        ...
        let borrow = utils.toWei(500);
        await Levex.marginTrade(0, false, true, deposit, borrow, 0, Uni2DexData, {from: trader});
    })
    it("LONG Token0, Init Price, Close Succeed ", async () => {
        ...
        let marginTradeTx = await Levex.marginTrade(pairId, false, true, deposit, borrow, 0, Uni2DexData, {from: trader});
        ...
        let closeTradeTx = await Levex.closeTrade(0, false, tradeBefore.held, 0, Uni2DexData, {from: trader});
    ...
    it("LONG Token0, Price Diffience>10%, Not update pirce, Long Again", async () => {
        ...
        // Trader to approve Levex to spend
        let deposit = utils.toWei(400);
        await token1.approve(Levex.address, utils.toWei(100000), {from: trader});
        // Saver deposit to pool1
    ...
    it("LONG Token0, Price Diffience>10%,Update price Long Again", async () => {
        ...
        await Levex.marginTrade(pairId, false, true, deposit, 0, 0, Uni2DexData, {from: trader});
        ...
        //
        let marginRatio = await Levex.marginRatio(trader, 0, 0, Uni2DexData);
        m.log("Margin Ratio current:", marginRatio.current / 100, "%");
        m.log("Margin Ratio havg:", marginRatio.hAvg / 100, "%");
    ...
    it("LONG Token0, Price Diffience>10%, 60s Later, Liquidation", async () => {
        ...
        //set price 0.5
        await gotPair.setPrice(token0.address, token1.address, 50);
        let marginRatio0 = await Levex.marginRatio(trader, pairId, 0, Uni2DexData);
        ...
        await assertThrows(Levex.liquidate(trader, pairId, 0, 0, utils.maxUint(), Uni2DexData, {from: liquidator2}), 'MPT');
        ...
        m.log("priceData1: \t", JSON.stringify(priceData1));
        //
        let marginRatio1 = await Levex.marginRatio(trader, pairId, 0, Uni2DexData);
    ...
    it("LONG Token0, Price Diffience>10%,Update Price, Liquidation", async () => {
        ...
        await assertThrows(Levex.liquidate(trader, pairId, 0, 0, utils.maxUint(), Uni2DexData, {from: liquidator2}), 'MPT');
        await advanceMultipleBlocksAndTime(1000);
        let updatePriceTx = await Levex.updatePrice(pairId, Uni2DexData, {from: accounts[2]});
        m.log("V2 UpdatePrice Gas Used: ", updatePriceTx.receipt.gasUsed);

        assert.equal((await Levex.markets(pairId)).priceUpdater, accounts[2]);
        let priceData1 = await dexAgg.getPriceCAvgPriceHAvgPrice(token0.address, token1.address, 60, Uni2DexData);
        m.log("priceData1: \t", JSON.stringify(priceData1));
        await advanceMultipleBlocksAndTime(1000);
        await gotPair.sync();
    ...
    it("LONG Token0, Update Price, Discount", async () => {
        ...
        await Levex.updatePrice(pairId, Uni2DexData, {from: trader});
        await Levex.marginTrade(pairId, false, true, deposit, borrow, 0, Uni2DexData, {from: trader});
        let tradeBefore = await Levex.activeTrades(trader, pairId, 0);
        m.log("Trade.held:", tradeBefore.held);
        assert.equal(tradeBefore.held, "887336915523826444724");
        assertPrint("Insurance of Pool1:", '668250000000000000', (await Levex.markets(pairId)).pool1Insurance);
    })
    ...
    it("MarginTrade For trader2", async () => {
        ...
        m.log("toBorrow from Pool 1: \t", borrow);
        await advanceMultipleBlocksAndTime(200);
        await Levex.updatePrice(pairId, Uni2DexData, {from: trader2});
        await assertThrows(Levex.marginTradeFor(trader2, pairId, false, true, deposit, borrow, 0, Uni2DexData, {from: trader}), 'OLO');
        await Levex.setOpLimitOrder(trader);
        await Levex.marginTradeFor(trader2, pairId, false, true, deposit, borrow, 0, Uni2DexData, {from: trader});
        let tradeBefore = await Levex.activeTrades(trader2, pairId, 0);
        m.log("Trade.held:", tradeBefore.held);
        assert.equal(tradeBefore.held, "887336915523826444724");
        await Levex.closeTrade(pairId, false, tradeBefore.held, 0, Uni2DexData, {from: trader2});
        let tradeAfter = await Levex.activeTrades(trader2, pairId, 0);
        assert.equal(tradeAfter.held, "0");
        ...
        pool1 = await LPool.at((await Levex.markets(pairId)).pool1);
        await token1.approve(await pool1.address, saverSupply, {from: saver});
        await pool1.mint(saverSupply, {from: saver});
        ...
        await advanceMultipleBlocksAndTime(200);
        await Levex.updatePrice(pairId, Uni2DexData, {from: trader2});
        await weth.approve(Levex.address, deposit, {from: trader});

        await Levex.marginTradeFor(trader2, pairId, false, false, deposit, borrow, 0, Uni2DexData, {from: trader});
        tradeBefore = await Levex.activeTrades(trader2, pairId, 0);
        m.log("Trade.held:", tradeBefore.held);
    ...
    it("CloseTrade For trader2", async () => {
        ...
        m.log("Trade.held:", tradeBefore.held);
        assert.equal(tradeBefore.held, "887336915523826444724");
        await assertThrows(Levex.closeTradeFor(trader, pairId, false, tradeBefore.held, 0, Uni2DexData, {from: trader2}), 'OLO');
        await Levex.setOpLimitOrder(trader2);

        await Levex.closeTradeFor(trader, pairId, false, tradeBefore.held, 0, Uni2DexData, {from: trader2})
        let tradeAfter = await Levex.activeTrades(trader, pairId, 0);
        assert.equal(tradeAfter.held, "0");
    ...
    it("LONG Token0, Close Amount>=99.999%, Close 100% default", async () => {
        ...
        await Levex.closeTrade(0, false, close_99, 0, Uni2DexData, {from: trader});
