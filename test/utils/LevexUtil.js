...
const xLVX = artifacts.require("XLVX");
...
exports.createController = async (admin, LVXToken, wChainToken, xLVXToken) => {
    let instance = await Controller.new();
    let controller = await ControllerDelegator.new(LVXToken ? LVXToken : zeroAddr,
        xLVXToken ? xLVXToken : zeroAddr,
...
exports.createLevex = async (controller, admin, dexAgg, xLVX, depositTokens) => {
    let LevexV1Lib = await LevexV1Lib.new();
    await LevexV1.link("LevexV1Lib", LevexV1Lib.address);
    let delegate = await LevexV1.new();
    let Levex = await LevexDelegator.new(
        controller,
        ...
        admin,
        delegate.address);
    return await LevexV1.at(Levex.address);
}

exports.createXLVX = async (LVX, admin, dev, dexAgg, account) => {
    let delegatee = await xLVX.new();
    let xLVXInstance = await xLVXDelegator.new(LVX, dexAgg, 5000, dev, admin, delegatee.address, {from: account == undefined ? admin : account});
    return xLVX.at(xLVXInstance.address);
}
...
exports.createVoteBySigMessage = (govAddress, proposalId, support, chainId) => {
    ...
    const domain = {name: 'Levex Governor Alpha', chainId, verifyingContract: govAddress};
    support = !!support;
...
exports.initEnv = async (admin, dev) => {
    ...
    let LVXToken = await this.createToken("Lvr");
    let usdt = await this.createToken("USDT");
    let controller = await this.createController(admin, LVXToken.address);
    ...
    let Levex = await LevexDelegator.new(controller.address, uniswapFactory.address, priceOracle.address, admin);
...
exports.getCall1inchSwapData = (router, fromToken, toToken, receiver, amount, minReturnAmount) => {
    let callData = router.contract.methods.swap("0x18101Ac1d35230F1A3c005e2aBaAEb25caE79e7f", [fromToken, toToken, "0x18101Ac1d35230F1A3c005e2aBaAEb25caE79e7f", receiver, amount, minReturnAmount, 4], "0x",
        ...
        "0000000000000000000e9e7cea3dedca5984780bafc599bd69add087d5680a06c4eca27e9e7cea3dedca5984780bafc599bd69add087d561111111254eeb25477b68fb85ed929f73a960582").encodeABI();
    return "0x1500000002" + callData.substring(2);
}