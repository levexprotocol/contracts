...
const {toWei, assertPrint, assertThrows} = require("./utils/LevexUtil");
...
contract("LPoolDelegator", async accounts => {
    ...
    it("Supply -> borrow -> supply more -> raise -> borrow more -> redeem partial -> repay -> redeem all", async () => {
        ...
        m.log("logging-totalBorrows--cashs", await erc20Pool.totalBorrows(), await erc20Pool.totalReserves(), await erc20Pool.getCash())
    ...
    it("Repay more and liquidate test", async () => {
        ...
        await erc20Pool.repayBorrowEndByLevex(accounts[3], 1000 * 1e10, {from: accounts[1]});
