...
const {toWei, toETH, assertThrows,} = require("./utils/LevexUtil");
...
contract("FarmingPools", async accounts => {
    ...
    let day1 = 24 * 60 * 60;
    beforeEach(async () => {
        LVXToken = await MockERC20.new('LVXToken', 'LVX');
        ...
        farmingPools = await FarmingPool.new(LVXToken.address, admin);
        await LVXToken.mint(admin, toWei(10000));
    ...
    it("One pool and one account: ", async () => {
        ...
        await LVXToken.mint(farmingPools.address, toWei(10000));
    ...
    it("Two pool and one account: ", async () => {
        ...
        await LVXToken.mint(farmingPools.address, toWei(30000));
    ...
    it("One pool exit: ", async () => {
        ...
        assert.equal(toWei(1000).toString(), await stakeToken1.balanceOf(stakeAcc1));
        assert.equal(amount.toString(), await LVXToken.balanceOf(stakeAcc1));
    });
    ...
    it("One pool distribute more: ", async () => {
        ...
        amount = await farmingPools.earned(stakeToken1.address, stakeAcc1);
        //more
        await LVXToken.mint(farmingPools.address, toWei(20000));
